...
class GNucleus : public TNamed{
 ...
 public:
  GNucleus() { };					              // Should not be use, here so we can write things to a root file.
  GNucleus(const char* symbol);                                       // Creates a nucleus based on symbol and sets all parameters from mass.dat
  GNucleus(int Z, int N, double mass, const char* symbol);            // Creates a nucleus with Z, N, mass, and symbol
  GNucleus(int Z, int N, const char* MassFile = 0);                   // Creates a nucleus with Z, N using mass table (default MassFile = "mass.dat")
  ...
  void SetSymbol(const char*);  			// Sets the atomic symbol for the nucleus
   ...
   void AddTransition(GTransition *tran);
  ...
  //Bool_t RemoveTransition(Int_t idx);
  GTransition *GetTransition(Int_t idx);
 ...
 private:
  ...
  void SetName();
   ...
   ClassDef(GNucleus,1);				// Creates a nucleus with corresponding nuclear information
};
