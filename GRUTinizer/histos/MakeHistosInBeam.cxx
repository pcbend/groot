...
void MakeHistograms(TRuntimeObjects& obj) {
  ...
  //Removed for ANL Gretina+FMA by BRL
  if(!s800Sim)
    return;
  ...
  for(unsigned int x=0; x<gretina->Size(); x++){
    ...
    if(hit.GetCoreEnergy() > energyLlim &&
      ...
      calorimeterEnergy_gaus += hit.GetCoreEnergy()*gRandom->Gaus(1,sigma_res);
    ...
    obj.FillHistogram("energy", "overview_gaus",
		      energyNChannels, energyLlim, energyUlim,
		      hit.GetCoreEnergy()*gRandom->Gaus(1,sigma_res),
		      100, 0, 100, hit.GetCrystalId());
    ...
    for(int i=0; i<nBetas; i++){
      ...
      Double_t e1 = hit.GetDoppler(betas[i])*gRandom->Gaus(1,sigma_res);
      for(unsigned int y=x+1; y<gretina->Size(); y++){

	TGretinaHit hit2 = gretina->GetGretinaHit(y);
	Double_t e2 = hit2.GetDoppler(betas[i])*gRandom->Gaus(1,sigma_res);
	...
	obj.FillHistogram("energy",
      ...
      obj.FillHistogram("energy",
			...
			energyNChannels, energyLlim, energyUlim,
			hit.GetDoppler(betas[i])*gRandom->Gaus(1,sigma_res));
  ...
  while(hits.size() > 0){
    ...
    for(unsigned int i = 0; i < cluster.size(); i++){
      ...
      addbackEnergy_gaus +=
	cluster[i].GetCoreEnergy()*gRandom->Gaus(1,sigma_res);
